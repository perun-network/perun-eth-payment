...
func newChannel(client *Client, ch *client.Channel) *Channel {
	...
	go ch.Watch(channel) // nolint: errcheck
	return channel
}
...
func (c *Channel) Balance() Balance {
	...
	return Balance{
		My:    MakeAmountFromWEI(c.state.Balances[0][c.ch.Idx()]),
		Other: MakeAmountFromWEI(c.state.Balances[0][1-c.ch.Idx()]),
	}
}
...
func (c *Channel) handleUpdate(update client.ChannelUpdate, responder *client.UpdateResponder) {
	...
	defer c.mtx.Unlock()
	me := c.ch.Idx()
	received := new(big.Int).Sub(update.State.Balances[0][me], c.state.Balances[0][me])
